...
public class AuthorizationController : Controller
{
    ...
    public async Task<IActionResult> Authorize()
    {
        ...
        else
        {
            ...
            _logger.LogInformation("[DEBUG] Authorize hit. Browser sent these cookies: {Cookies}", string.IsNullOrEmpty(cookieList) ? "NONE" : cookieList);
            ...
            {
                _logger.LogWarning("No SSO Session detected. Scheme: {Scheme}. Failure: {Failure}", 
                    IdentityConstants.ApplicationScheme,
                    result.Failure?.Message ?? "Cookie missing or invalid");
            }
        }
